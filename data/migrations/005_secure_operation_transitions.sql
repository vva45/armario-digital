...
begin
  ...
  if p_front_path<>auth.uid()::text||'/'||op.garment_id::text||'/'||op.id::text||'/frontal'
     or not exists(select 1 from jsonb_array_elements(op.manifest) x where x->>'side'='frontal' and x->>'type'=p_front_type and (x->>'size')::bigint=p_front_size)
     or ((p_back_path is null) <> not exists(select 1 from jsonb_array_elements(op.manifest) x where x->>'side'='trasera'))
     or (p_back_path is not null and (p_back_path<>auth.uid()::text||'/'||op.garment_id::text||'/'||op.id::text||'/trasera'
       or not exists(select 1 from jsonb_array_elements(op.manifest) x where x->>'side'='trasera' and x->>'type'=p_back_type and (x->>'size')::bigint=p_back_size)))
    then raise exception 'Confirmación incompatible'; end if;
...
begin
 ...
 if p_operation_id is not null and (
   (p_new_front_path is not null and (p_new_front_path<>auth.uid()::text||'/'||op.garment_id::text||'/'||op.id::text||'/frontal'
     or not exists(select 1 from jsonb_array_elements(op.manifest) x where x->>'side'='frontal' and x->>'type'=p_front_type and (x->>'size')::bigint=p_front_size)))
   or (p_new_back_path is not null and (p_new_back_path<>auth.uid()::text||'/'||op.garment_id::text||'/'||op.id::text||'/trasera'
     or not exists(select 1 from jsonb_array_elements(op.manifest) x where x->>'side'='trasera' and x->>'type'=p_back_type and (x->>'size')::bigint=p_back_size)))
   or jsonb_array_length(op.manifest)<>(case when p_new_front_path is null then 0 else 1 end)+(case when p_new_back_path is null then 0 else 1 end)
 ) then raise exception 'Confirmación incompatible'; end if;
...
returns jsonb language sql security invoker set search_path=public stable as $$
  select jsonb_build_object('bytes',coalesce(sum(i.byte_size),0),'images',count(*))
  from wardrobe_images i where i.owner_id=auth.uid()
$$;
revoke all on function wardrobe_storage_summary() from public,anon;
grant execute on function wardrobe_storage_summary() to authenticated;
